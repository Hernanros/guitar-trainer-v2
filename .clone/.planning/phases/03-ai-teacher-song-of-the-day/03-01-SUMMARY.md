...
Device walkthrough on iOS Simulator/device with local Postgres + FastAPI. See checkpoint below.
...
- **Issue:** The original apiClient.ts comment said "no direct calls to `fetch()`" which made the grep `grep -c "fetch(" apiClient.ts` return 2 instead of the expected 1
...
- **Issue:** Renaming to `ONBOARDING_LOADER_MESSAGES` still contained the pattern `LOADER_MESSAGES` in the identifier
...
- **Issue:** `onSuccess: (fresh) =>` and `onError: async (err) =>` had no explicit types, causing implicit `any` errors under `strict` mode
...
- **Found during:** Task 3 — TypeScript check pass
- **Issue:** `const _r0: 0 = leftWhenRerolled` fails because `assertRerollsLeft` return type is `0 | 1`, not narrowed to literal `0`
...
- `rerolls_left` is hardcoded as `today.rerolled ? 0 : 1` on the mobile side (not read from server response field) — server does return `rerolls_left: int` in `TodaySongResponse` but the mobile Today tab derives it locally. This is correct for single-user POC where quota is 1/day; Slice C or multi-device support will consume the server field.
...
- `server/app/selectors/today_song.py` — exists (Task 2)
- `mobile/src/api/todaySong.ts` — exists (Task 3)
- `mobile/src/components/SongOfDayCard.tsx` — exists (Task 3)
- `mobile/src/components/FletcherLoader.tsx` — exists (Task 3)
- `mobile/src/components/FromTheBankTag.tsx` — exists (Task 3)
- Commit `97396d0` — Task 1 (verified via git show)
- Commit `c52fd2b` — Task 2 (verified via git log)
- Commit `6a28e11` — Task 3 (verified via git log)
